...
import sys
def greet(): print("Welcome to", __title__, "version", __version__)
...
if os.name == "nt":
	...
	try:
		...
		def RunURL(url):
			print("Error: cannot open URL `%s'" % url)
...
def ParsePDF(filename):
	...
	try:
		try:
			...
			print ("Note: error in file produced by pdftk, hyperlinks disabled.", file=sys.stderr)
	...
	if not IsZoomed:
		...
		try:
			...
			if not ZoomWarningIssued:
				...
				print ("exhausted, or there is no support for large textures (%dx%d). If you really" % (TexWidth * 2, TexHeight * 2), file=sys.stderr)
				print ("need high-res zooming, please try to run Impressive in a smaller resolution", file=sys.stderr)
				print ("using the -g command-line option.", file=sys.stderr)
				ZoomWarningIssued = True
			return
		DrawCurrentPage()
		IsZoomed = True
